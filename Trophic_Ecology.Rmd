...
You are probably already familiar or already probably heard about R. You may not be proficient in using it, and the aim of this class is to introduce some common and more advanced tools to analyse stable isotope in the field of trophic ecology. This class will not come back on general knowledge of what is an isotope and how they can be useful for ecology (see previous courses), but it will focus on more technical details which are how we analyse the data. In particular, we will navigate in R packages/functions allowing to visualize stable isotope data and we will go through common R packages used in trophic ecology. 

I will use Posit (ex RStudio) for the demo. If you have it installed on your computer, you can use it but make sure it is updated with the latest version of R/Posit. This class has also been optimized for a use with **posit** Cloud. You can register and use it  for free  with, of course, some limitation on computing time (25 hrs) and RAM (1GB). That should be okay largely enough for two days if you close it after the class. 

GUI of Posit/RStudio is pretty standard: script editor, console, environment, files - plots - packages windows. That's all we need for now.

### R Markdown to load and plot data

This is an R Markdown document. A kind of R script but with some 'bonus' that let you to annotate and explain what you did. Markdown is a simple formatting syntax for authoring HTML, PDF, and MS Word documents. For more details on using R Markdown see <http://rmarkdown.rstudio.com>.

We'll use this to compile and store all the r code used during the class. This way you can edit and improve the scripts as you go on and even copy relevant sections for other projects. This markdown is being saved as a html document but you can also open the markdown directly as you  see on my screen.
...
All data and code to replicate the analysis here are provided [here](https://github.com/vianneydenis/trophic-ecology.git). You can use this repository and knit the .Rmd file. Edits, suggestions, and comments are welcome.
...
First load the data associated with this workshop (the file is named *Dataset*). We will manipulate and transform this dataset as we work)
This file contains carbon and nitrogen data for two coral species (one black coral and one gorgonian coral) and their possible food sources. Part of the data are published and available at: https://www.sciencedirect.com/science/article/pii/S0272771418310606 

Generally speaking, your data should already have been tidied up, probably in Excel, so you removed any of the additional lines at the start that often come back from analytical laboratories. R uses 'raw' format. It means you need to keep your datasheet very simple (no color or any other fancy format). Just be boring, that's good for R. As with all data files, your first line (row) should be the names of each of the columns. When naming columns, it is important to not use any unusual, non-standard characters, as these can confuse the functions that are parsing (reading and processing) the data file: if you need to separate words in a column header you should use "_" or ".", e.g. "delta_C" or "delta.C" but not "delta/C". It is generally not recommended to have spaces.
...
Get familiar with the different variables. You can further examine the structure of this data `str(Dataset)`. Importantly, you can note that information on the station and depth have been merged together. Also note the presence of missing data, mostly at the `tail` of the data set. For the purpose of this demonstration, we will remove useless variables and make a simplified dataset. 
...
Visualizing the data is often the next step. You can generate histograms of the data very easily. Let's compare isotope ratios between the two coral species. Here we are primarily interested in d13C values of *A. subpinnata* and *E. cavolini*.
...
- Carbonate: At first, we may want to test whether there is evidence for an effect of inorganic carbon on the carbon isotope values. If this was a factor we would expect to see a positive relationship between %C and d13C. Here, the  d13C data we got are already from samples that have been acidified and in which carbonates have been remove. Yet, for the fun we can make the plot: 
...
Samples have been acidified and inorganic carbon removed.
...
Alternatively we can identify whether any individuals / populations have C:N values above a threshold. A CN ratio of 7 is common for corals ([Fujii et al. 2020](https://www.mdpi.com/2076-2607/8/8/1221); [Hoegh-Guldberg et al. 2004](https://www.int-res.com/abstracts/meps/v280/p105-114/); [Hsu et al. (2020)](https://www.frontiersin.org/articles/10.3389/fmars.2020.606601/full?ref=https://githubhelp.com); [Sturaro et al. (2019)](https://besjournals.onlinelibrary.wiley.com/doi/full/10.1111/1365-2435.13924))
...
Lipid removal may have been beneficial, but in corals C:N is generally high independently of lipid removal. This is also taxon specific, so some preliminary test are usually advised. Know your beast ! There is a lot of literature on the topic. Among others, check [Sturaro et al. (2019)](https://analyticalsciencejournals.onlinelibrary.wiley.com/doi/full/10.1002/rcm.8663) and its supplementary information for a details methodology.



We went through these comparisons individually here, but you can use pairs plots to summarize much of this information. As ever, there is a ‘Hadleyverse' option, `ggpairs`, in the GGally package. These are coarse plots but an excellent way to take a first look at your data.
...
Here the warning is important as it indicates that you have missing data. As for an examples, none of the sediment samples were measured for both d15N and d13C. The x- and y-axes will be adjusted according to these values but no data will be plotted. 
...
The initial question in study was whether *A. subpinnata* and *E. cavolini* used different resources (i.e. had different isotope ratios) in the two seasons. Do they? Lets start with a biplot to visualize the difference between the two coral species.
...
```{r, fig.show = "hold", out.width = "60%", fig.align = "center", fig.cap="Two sources mixing model explained - Adapted from Snowy illustration by Brian Hayden", echo=F}

knitr::include_graphics("Figures/2_sources.png")
```

The first step is to identify the baseline values. Here we will use pico-nanoplanton and POM (sediment) as pelagic/oceanic and littoral/coastal end-members respectively.
...
You just ran your first and simple stable isotope mixing model. At both seasons *A. subpinnata* assimilates predominantly pelagic resources compared with *E. cavolini*. Both species are segregated along a pelagic-littoral axis.

### Mixing models - three or more sources 


Up to now we have looked at relatively simple two-end members mixing models. The previous 2 sources - 2 tracers model had several limitations:
...
`simmr` and `MixSIAR` are both packages designed to solve mixing equations for stable isotopic data within a Bayesian framework. Here `MixSIAR` has an advantage as it allows to run multiple models at the same time such as here the two seasons. Therefore for the purpose of the demonstration today, we will focus our effort on using the package `MixSIAR`.

#### MixSIAR - solving complex stable isotope mixing models

`MixSIAR` was developed by Brian Stock and is the result of a collaboration between Brice Semmens who developed `MixSIR` and Parnell et al. who developed `SIAR`. It is an incredibly powerful tool which can assess differences in resource use among different communities in a single model. It can be very complicated, and some of you better than I am in using this package. For the purpose of this demonstration we will just give a **brief introduction** to how to analyze our data set.

Here again, the code and notes are mostly extracted from materials shared by Brian Haydens. Himself credited Brian Stock and Brice Semmens who wrote a wonderful manual to accompany `MixSIAR` package [MixSIAR Manual](https://github.com/brianstock/MixSIAR/blob/master/Manual/mixsiar_manual_small.pdf). You can find it along with additional details about the package on GitHub (https://github.com/brianstock/MixSIAR) and the package is presented in Brians 2018 paper (https://peerj.com/article/5096.pdf). 
...
- Write mixture data (i.e consumers)
...
Bayesian analyses require priors, and MixSIAR includes a `plot_prior` function to plot the prior on the mixture (diet) proportions (at the highest hierarchical level, p.global). The prior represents our **knowledge** about the proportions before we consider the biotracer data. A natural tendency is to want a flat/"uninformative" prior, where all values between 0 and 1 are equally likely. However, because proportions are not independent, there is no truly uninformative prior (e.g. the histograms are not flat). The best we can do with the Dirichlet distribution is set $\alpha$ = c(1,1,1,1), which is uninformative on the simplex. In other words, all combinations of the proportions are equally likely. See the section titled "Constructing informative Bayesian priors" in the forthcoming MixSIAR paper.

Because the mean of the "uninformative" prior, $\alpha$ = c(1,1,1,1), is $\frac{1}{n.sources}$, we also call it the generalist prior. This reflects two facts: 1) it is not really uninformative, and 2) for weakly informative data it shifts the posterior towards a generalist diet, $p_1 = p_2 = p_3 = p_4 = \frac{1}{4}$. The amount of shift depends on the informativeness (quality and quantity) of the data.
...
+ DARK GREY = "uninformative"/generalist (alpha = 1 OR alpha = 1/n.sources )


Following is an example of informative prior:
...
```
![](Plots/informative_prior_plot.png)

##### **Write JAGS model file**
    
Write the JAGS model file (define model structure). The model will be saved as `model_filename` ("MixSIAR_model.txt" is default, but you may want to change it if you create many different models). It will define the overall structure of the Markov Chain Monte Carlo (MCMC) model.
...
The differences between "Residual * Process", "Residual only", and "Process only" are explained in [Stock and Semmens (2016)](https://esajournals.onlinelibrary.wiley.com/doi/10.1002/ecy.1517). If you have only 1 mix datapoint, you have no information about the mixture/consumer variability. In this case, we use the original MixSIR error model (which does not fit a residual error term). This is the same behavior as `siarsolo` in SIAR. Here, like in the **Wolves** example (see MixSIAR manual) we want the "Residual * Process" error option. 
...
Our model is written, we now want to run it.

##### **Run model**

There are several pre-existing options to run your MCMC model: 
...
Alternatively, you can also set custom MCMC parameters, e.g:

```{r, eval=TRUE}
run <- list(chainLength=1000, burn=500, thin=1, chains=3, calcDIC=TRUE)
```

A good idea to use `run = "test"` at first to check if 1) all the data are loaded correctly and if 2) the model is specified correctly:
...
More on MCMC at https://youtu.be/vTUwEu53uzs?feature=shared.
...
Here again, the first task is to take back our original data set and to make some basic manipulations. The SIBER package isn't quite as flexible as some of the others we have worked with so far. For SIBER analysis we need to code all data as *community* (in this case `Season`) and *group* (in this case 'Category'). In addition, d13C and d15N **MUST** be named iso1 and iso2.

First an overview of the data set:
...
Then some magic to start running the analysis! The `SIBER` package requires the data set in a specific format, *i.e.* a SIBER object. After renaming the variables, we can use the function `createSiberObject()` to create this object. 
...
Okay, so we have now visualized the isotopic niche of each population but how about getting some data. We can obtain the convex hull area (TA), Standard Ellipse Area (SEA) and Standard Ellipse Area corrected (SEAc) based on the maximum likelihood estimates of the means and covariance matrices of each group.
...
These data are just **single value** however, e.g. geometric measurements of the area of each hull or ellipse. If want to know how reliable those data are we need to calculate a **confidence interval**. The best way to do that is by calculating the Bayesian ellipse (SEAb).

To do this we run the SIBER model again, first we need to provide model parameters and priors, we enter the default options are below
...
The model ran successfully, next let's look at the output:
...
Visualizing the data is all well and good, show me the data (note: can skip this part)!
...
Next, we can already see differences between each season but how reliable are they? To find out we run the SIBER model and calculate the Bayesian community metrics
...
Alternatively we can pull out a specific metric of interest and compare it between lakes. Here we compare **TA** and **Nr**
