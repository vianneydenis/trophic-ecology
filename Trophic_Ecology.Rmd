...
- Carbonate: First we want to test whether there is evidence for an effect of inorganic carbon on the carbon isotope values. If this was a factor we would expect to see a positive relationship between %C and d13C. Why?
...
There is no strong evidence of inorganic C in these samples, so we can continue. We would expect a positive relationship if carbonate effect.
In addition, the sammples have already been acidified. Still it is a good pratice to check.  

- Lipid: Lipids contain carbon but no nitrogen AND are 13C depleted relative to protein (i.e. muscle). So, lipid rich samples will be depleted in 13C, in extreme cases this may bias our interpretation of their position in the food web.

Pure protein has an elemental C:N ratio of 3 (2.8 - 3.2), C:N values above this are indicative of lipids in the matrix. There are two otpions to deal with this: you can remove lipids from the sample using repeated washes in a 2:1 chloroform:methanol solution or using a lipid correction model (not suitable for invertebrates).
...
Lipid removal could have been beneficial, but in corals C:N is generally high independently of lipid removal.
